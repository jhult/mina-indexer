use crate::{
    ...
    stream::{canonical_items_manager::CanonicalItemsManager, events::Event, payloads::CanonicalBlockLogPayload},
};
    ...
    async fn handle(&self, _myself: ActorRef<Self::Msg>, msg: Self::Msg, state: &mut Self::State) -> Result<(), ActorProcessingErr> {
        match msg {
            Event::BlockCanonicityUpdate(payload) => {
                {
                    let mut manager = self.canonical_items_manager.lock().await;
