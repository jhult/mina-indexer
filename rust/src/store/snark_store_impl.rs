...
impl SnarkStore for IndexerStore {
    ...
    fn top_snark_workers_iterator<'a>(&'a self, mode: IteratorAnchor) -> DBIterator<u64, PublicKey> {
        self.iterator(SNARK_TOP_PRODUCERS_SORT)
    }

    fn snark_fees_iterator<'a>(&'a self, mode: IteratorAnchor) -> DBIterator<(u64, u32, PublicKey, BlockHash), u32> {
        self.iterator(SNARK_WORK_FEES)
    }
    ...
    fn snark_prover_iterator<'a>(&'a self, mode: IteratorAnchor) -> DBIterator<(PublicKey, u32, u32), SnarkWithCanonicity> {
        self.iterator(SNARK_WORK_PROVER)
    }
