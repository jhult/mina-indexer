...
async fn merge() -> anyhow::Result<()> {
    ...
    let (extension_type, _) = state.add_block_to_witness_tree(&leaf0_block, true, true)?;
    ...
    let (extension_type, _) = state.add_block_to_witness_tree(&leaf1_block, true, true)?;
    assert_eq!(extension_type, ExtensionType::DanglingNew);
    ...
    let leaves0: HashSet<Block> = root_branch.leaves().into_iter().collect();

    assert_eq!(HashSet::from([leaf0, leaf1]), leaves0.into_iter().collect());
